...
- 그룹 내 모든 프로세스에게 시그널을 보낼 수 있다.
- pgid로 구분되며 각 프로세스 그룹마다 그룹 리더가 있다.
- 프로세스 그룹 ID는 프로세스 그룹 리더의 pidd이다.
- 프로세스 그룹 리더가 종료되어도 프로세스 그룹은 남는다.

2. 세션
- 사용자가 시스템에 로그인하면 로그인 프로세스는 새로운 세션을 생성한다.(setsid 호출)
- 로그인 셸 프로세스(bash 인스턴스)는 새로운 세션의 리더가 된다. 세션 리더의 pid는 세션 ID로 사용된다.
- 세션은 하나 이상의 프로세스 그룹이 들어 있는 집합이다.
- 세션은 로그인한 사용자 활동을 처리하며 사용자의 터미널 입출력을 다르는 tty장치로 명시되는 제어 터미널과 사용자 사이를 연결한다.
- 세션에 속한 프로세스 그룹은 하나의 포어 그라운드 프로세스 그룹, 0개 이상의 백그라운드 프로세스 그룹으로 나뉜다.
- 사용자가 터미널을 종료하게 되면 포어그라운드 프로세스 그룹 내 모든 프로세스에 SIGQUIT 시그널이 전달된다.
- 사용자가 터미널에서 Ctrl + C 같은 취소 명령을 입력한 경우 포어그라운드 프로세스 그룹 내 모든 프로세스에 SIGINT 시그널이 전달된다.
- 시스템에는 많은 세션이 존재한다.
: 사용자의 로그인 세션, 데몬과 같이 사용자의 로그인 세션과 연결되지 않은 프로세스를 위한 세션도 있다.

3. 데몬
- 데몬은 종료될지 모르는 다른 세션과 관계된 문제를 피하기 위해 독자적인 세션을 만든다.
- 백그라운드에서 수행되며 제어 터미널이 없는 프로세스이다.
- 일반적으로 부팅시에 시작되며 root혹은 다른 특수한 사용자 계정 권한으로 실행되어 시스템 수준의 작업을 수행한다.
- 반드시 init 프로세스의 자식(fork이후 부모를 종료)이어야 하며 터미널과 연결되면 안된다.
*/

#include <fcntl.h>
#include <linux/limits.h> // NR_OPEN(1024)
...
void make_daemon()
{
	...
	// 새로운 프로세스 생성
	// 부모는 exit를 호출해서 종료한다.
	// 이를 통해 데몬 프로세스가 프로세스 그룹 리더가 되지 않도록 한다.
	switch(pid = fork()) {
	case -1:
		errexit("fork");
		break;
	case 0:
		// 데몬이 될 자식이 새로운 세션을 생성하여 세션과 프로세스 그룹의 리더가 됨으로써
		// 데몬 프로세스가 제어 터미널(부모가 있던)과 연관되지 않도록함.
		if(setsid() == -1)
			errexit("setsid");
		break;
	default:
	...
	// 파일 디스크립터 0, 1, 2를 /dev/null로 리다이렉트한다.
	// 위에서 0부터 모두 닫았기 때문에 아래에서 자동으로 0, 1, 2로 지정됨.
	open("/dev/null", O_RDWR); // 0
	dup(0); // 0을 복사해서 1로 리다이렉트
	dup(0); // 0을 복사해서 2로 리다이렉트


//	// 리눅스는 daemon 함수를 제공하여 위의 과정을 한번에 수행할 수 있도록 함.
//	if(daemon(0, 0) == -1)
//		errexit("daemon");
